    ...
    try {
      const idpHint =
        provider === "google"
          ...
          : (import.meta.env.VITE_KEYCLOAK_GITHUB_IDP_HINT || "github");

      const redirectUri = `${window.location.origin}/dashboard`;
      const baseLoginUrl = await keycloak.createLoginUrl({ redirectUri });
      const loginUrl = new URL(baseLoginUrl);

      // Force broker selection so Keycloak skips its local login form.
      loginUrl.searchParams.set("kc_idp_hint", idpHint);
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-3" style={{
            ...
            <Button
              type="button"
              variant="outline"
              className="h-10 text-xs gap-2 bg-background/30 hover:bg-background/60 hover:border-primary/30 transition-all duration-300 group"
